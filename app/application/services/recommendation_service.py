...
class RecommendationService:
    def __init__(self):
        ...
        self.rust_keywords = ["rust", "ownership", "borrow checker", "unsafe", "tokio", "async", "cargo"]
        self.cpp_keywords = ["c++", "cpp", "smart pointers", "templates", "move semantics", "raii", "std::"]

    def _analyze_historical_performance(self, posts: List[Post]) -> Dict[str, Any]:
        if not posts:
            return {"top_themes": ["technical_deep_dive"], "best_tone": "pragmatic_balance", "avg_engagement": 0.0}

        scored_posts = []
        for post in posts:
            engagement = (post.like_count or 0) + (post.comment_count or 0) * 3 + (post.repost_count or 0) * 2
            if post.content and len(post.content) > 50:
                themes = self.text_analyzer.detect_themes(post.content, getattr(post, "comments", None))
                sentiment = self.sentiment_analyzer.analyze(post.content, getattr(post, "comments", None))
                score = engagement * (1 + sentiment.get("pragmatic", 0) - sentiment.get("tribalism", 0) * 0.5)
                scored_posts.append({"themes": themes, "sentiment": sentiment, "score": score, "content": post.content[:300]})
        ...
        return {"top_themes": [t[0] for t in top_themes], "best_tone": best_tone, "avg_engagement": round(avg_eng, 1)}
    ...
    def generate_next_post_plan(self, historical_posts: List[Post], profile: Optional[Profile] = None) -> Dict[str, Any]:
        ...
        if domain == "rust":
            topic = "How Rust's ownership model eliminates entire classes of bugs that still plague modern C++ codebases"
            structure = [
            ...
            hook = "We spent three weeks chasing a use-after-free in production. The fix in Rust would have been impossible to write."
            length = 1650
        elif domain == "cpp":
            topic = "Modern C++ techniques that deliver Rust-level safety without forcing a full rewrite"
            structure = [
            ...
            hook = "Your C++ team is shipping faster than the Rust team next door. Here's exactly how they're doing it."
            length = 1550
        else:
            ...
            hook = "After shipping both Rust and C++ services at scale, the answer isn't what most engineering blogs claim."
            length = 1400
        ...
        plan = {
            ...
            "why_it_works": f"Historical data shows {best_tone} tone with {top_themes[0] if top_themes else 'technical'} themes drives the highest engagement in this domain.",
            "key_sections": [
