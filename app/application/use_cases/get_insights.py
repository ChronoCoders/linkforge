...
class GetProfileInsightsUseCase:
    def __init__(self, profile_repository: ProfileRepository, post_repository: PostRepository):
    ...
    async def execute(self, dto: AnalyticsRequestDTO) -> AnalyticsResponseDTO:
        ...
        posts = await self.post_repository.get_posts_for_training(limit=200) if dto.include_posts else []
        ...
        ml_recommendations = self.predictor.generate_recommendations(sentiment, theme_analysis.get("themes", []), num=3)
        ...
        response.recommendation = {
            ...
            "suggested_themes": [best_rec.get("theme", "technical_deep_dive")] if best_rec else ["technical_deep_dive"],
