...
class LinkedInScraper(BaseScraper):
    ...
    async def _extract_reactions(self, container: Locator) -> Dict[str, int]:
    ...
    async def _extract_visible_comments(
        self, container: Locator, max_comments: int = 4
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in items[:max_comments]:
                try:
                    text = (
                        await item.locator(".comments-comment-item-content").first.inner_text(
                            timeout=1500
                        )
                    ).strip()
                    author = ""
                    try:
                        author = (
                            await item.locator(".comments-post-meta__name").first.inner_text(
                                timeout=800
                            )
                        ).strip()
                    except Exception:
                    ...
                    try:
                        likes_text = await item.locator(
                            ".comments-comment-item__likes"
                        ).first.inner_text(timeout=800)
                        like_count = (
                            int(likes_text.split()[0].replace(",", "")) if likes_text else 0
                        )
                    except Exception:
                        pass
                    quality = min(
                        1.0, (len(text) / 280.0) * 0.6 + (0.4 if "?" in text or "!" in text else 0)
                    )
                    comments.append(
                        {
                            "text": text[:2800],
