...
class TextAnalyzer:
    ...
    POLARITY_WORDS: ClassVar[Dict[str, List[str]]] = {
    ...
    def _calculate_theme_confidence(self, text: str, keywords: List[str]) -> float:
        ...
        context_bonus = (
            0.2 if any(kw in normalized for kw in ["rust", "c++", "performance", "safety"]) else 0.0
        )
    ...
    def detect_themes_with_confidence(
        self, text: str, comments: Optional[List[Dict[str, Any]]] = None
    ) -> Dict[str, float]:
        combined = text
        if comments:
        ...
        for theme, keywords in self.THEME_KEYWORDS.items():
            conf = self._calculate_theme_confidence(combined, keywords)
            if conf > 0.15:
    ...
    def _advanced_polarization(self, text: str, comments: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def compare_to_historical(
        self, current_text: str, historical_posts: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def analyze(
        ...
        comparison = (
            self.compare_to_historical(text, historical_posts or []) if historical_posts else {}
        )
        return {
            "themes": themes,
