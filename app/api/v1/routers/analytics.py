...
async def get_next_post_recommendations(
    ...
    use_case = GetProfileInsightsUseCase(profile_repo, post_repo)
    try:
        result = await use_case.execute(AnalyticsRequest(profile_id=payload.profile_id, include_posts=True))
        return {
            "profile_id": payload.profile_id,
            "next_post_plan": getattr(result, "next_post_plan", {}),
            "ml_recommendations": getattr(result, "next_best_posts", []),
            "recommendation": getattr(result, "recommendation", {}),
            "recommendation_confidence": getattr(result, "recommendation_confidence", {})
        }
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
async def get_trends(
    ...
    use_case = GetProfileInsightsUseCase(profile_repo, post_repo)
    try:
        result = await use_case.execute(AnalyticsRequest(profile_id=payload.profile_id, include_posts=True, include_historical=True))
        return {
            "profile_id": payload.profile_id,
            "trend_data": getattr(result, "trend_data", []),
            "historical_comparison": getattr(result, "historical_comparison", {}),
            "post_performance_reports": getattr(result, "post_performance_reports", [])
        }
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
...
async def compare_posts(
    ...
    try:
        result = await use_case.execute(AnalyticsRequest(profile_id=payload.profile_id, include_posts=True))
        reports = getattr(result, "post_performance_reports", [])
        ...
        return {
            ...
            "comparison_summary": {
                "count": len(selected),
                "avg_engagement": sum(r.get("engagement_score", 0) for r in selected) / max(len(selected), 1),
